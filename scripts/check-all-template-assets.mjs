...
const slugs = readdirSync(liveRoot)
  .filter((name) => statSync(path.join(liveRoot, name)).isDirectory())
  .sort();
...
for (const slug of slugs) {
  ...
  console.log(`\\n[${slug}] html_files=${htmlFiles.length} refs=${refs.size} missing=${missing.length}`);
